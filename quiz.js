...
const QuizApp = {
    ...
    recordStat: function (c, cat, sub, isCorr) {
        ...
        // Course-level totals are historically cumulative
        this.stats[c].t++;
        if (isCorr) this.stats[c].c++; else this.stats[c].w++;
        
        if (!this.stats[c].bd) this.stats[c].bd = {};
        if (!this.stats[c].bd[cat]) this.stats[c].bd[cat] = {};
        
        // Unit-level breakdown is overwritten with the current attempt's results
